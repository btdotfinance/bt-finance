...
contract StrategyUSDTUNIFarm  {
    ...
    address constant public want = address(0x0d4a11d5EEaaC28EC3F61d100daF4d40471f1852);  //ETH/USDT

    address constant public token = address(0xdAC17F958D2ee523a2206206994597C13D831ec7);	//usdt
    address constant public Harvest = address(0xF8ce90c2710713552fb564869694B2505Bfc0846);
	address constant public fUNI = address(0x7DDc3ffF0612E75Ea5ddC0d6Bd4e268f70362Cff);
	address constant public pool = address(0x75071F2653fBC902EBaff908d4c68712a5d1C960);
    ...
    function deposit() public { 
		...
		uint256 _funi = IERC20(fUNI).balanceOf(address(this));
		if (_funi >0)
		{
		    IERC20(fUNI).safeApprove(pool,0);
		    IERC20(fUNI).safeApprove(pool, _funi);
		    pERC20(pool).stake(_funi);
		}
    }
    ...
    {
        uint256 _funi = _amount.mul(1e18).div(yERC20(fUNI).getPricePerFullShare());
		uint _before = IERC20(fUNI).balanceOf(address(this));
		if (_before < _funi) {
			_funi = _funi.sub(_before);
			require(_funi <= IERC20(pool).balanceOf(address(this)),"Insufficient Balance");
			pERC20(pool).withdraw(_funi);
		}
		
		_funi = IERC20(fUNI).balanceOf(address(this));
		yERC20(fUNI).withdraw(_funi);
    }
	...
	function balanceOfFUNI() public view returns (uint256) {
	    uint256 _funi = IERC20(fUNI).balanceOf(address(this));
		return _funi.add(IERC20(pool).balanceOf(address(this)));
	}
	

    function balanceOf() public view returns (uint256) {
        return balanceOfwant().add(balanceOfFUNI().mul(yERC20(fUNI).getPricePerFullShare()).div(1e18));
    }
